...
describe('P3-EFFECTIVE-SELLER', { concurrency: 1 }, () => {
  ...
  describe('T16/T17 — SAT token structure and expiration', () => {
    ...
    it('allows null sucursalId', () => {
      const token = signSellerAuthorization({ sellerId: 1, sessionUserId: 2, empresaId: 3, sucursalId: null })
      const decoded = jwt.decode(token)
      assert.strictEqual(decoded.bid, null)
    })

    it('rejects invalid sellerId', () => {
      assert.throws(() => signSellerAuthorization({ sellerId: -1, sessionUserId: 1, empresaId: 1 }), /positive integer/)
      assert.throws(() => signSellerAuthorization({ sellerId: 0, sessionUserId: 1, empresaId: 1 }), /positive integer/)
      assert.throws(() => signSellerAuthorization({ sellerId: 'abc', sessionUserId: 1, empresaId: 1 }), /positive integer/)
    })
    ...
    it('rejects expired SAT', () => {
      const token = jwt.sign(
        { sub: 1, sid: 2, eid: 3, bid: null, iat: Math.floor(Date.now() / 1000) - 600, exp: Math.floor(Date.now() / 1000) - 300 },
        TENANT_CONFIG.secret,
  ...
  describe('T19 — Seller token cannot authenticate session', () => {
    it('rejects SAT as Bearer token (wrong audience)', () => {
      ...
      // Simulate what requireAuth does: jwt.verify with session audience
      assert.throws(() => {
        jwt.verify(sat, TENANT_CONFIG.secret, {
          algorithms: [TENANT_CONFIG.algorithm],
          issuer: TENANT_CONFIG.issuer,
          audience: TENANT_CONFIG.audience  // session audience, NOT pos-seller-auth
        })
      }, /audience/)
    })
  ...
  describe('Ventas controller — seller resolution', () => {
    // We test the controller logic by importing and calling with mocked req/res.
    // The controller is large, so we extract the seller resolution logic conceptually
    // by testing the decision rules directly.

    it('T1 — no usuarioId in body → uses session user', () => {
      // Case A: bodyUsuarioId is null/undefined → usuarioId = req.usuario.id
    ...
    it('T1b — usuarioId matches session user → uses session user', () => {
      // Case B: bodyUsuarioId === req.usuario.id → usuarioId = req.usuario.id
      const sessionUserId = 100
    ...
    it('T20 — usuarioId different from session, no SAT → REJECT', () => {
      // Case C: bodyUsuarioId !== req.usuario.id, no SAT → must reject
      const sessionUserId = 100
    ...
    it('T2 — usuarioId different, valid SAT → uses seller from SAT', () => {
      // Case D: bodyUsuarioId !== req.usuario.id, valid SAT → seller = SAT.sub
      const sessionUserId = 100
      const bodyUsuarioId = 200
      const sat = signSellerAuthorization({ sellerId: 200, sessionUserId: 100, empresaId: 10, sucursalId: 30 })
      const result = resolveSellerId(bodyUsuarioId, sessionUserId, sat)
    ...
    it('T21 — SAT.sub != body.usuarioId → REJECT', () => {
      // SAT says seller is 200, but body claims seller is 300
      const sat = signSellerAuthorization({ sellerId: 200, sessionUserId: 100, empresaId: 10, sucursalId: 30 })
    ...
    it('T19b — SAT cannot bypass session auth', () => {
      // A SAT should not be accepted as a session token
      const sat = signSellerAuthorization({ sellerId: 200, sessionUserId: 100, empresaId: 10, sucursalId: 30 })

      // Try to verify as session token (wrong audience)
      assert.throws(() => {
        jwt.verify(sat, TENANT_CONFIG.secret, {
          algorithms: ['HS256'],
          issuer: TENANT_CONFIG.issuer,
  ...
  describe('Code integrity — key patterns', () => {
    ...
    it('ventas.controller.js no longer hardcodes req.usuario.id as usuarioId', () => {
      const src = fs.readFileSync(path.join(ROOT, 'jesha-pos-backend', 'src/modules/ventas/ventas.controller.js'), 'utf8')
      // The old line: const usuarioId = req.usuario.id
      // Should NOT exist as a standalone assignment anymore
    ...
    it('seller-auth.js uses distinct audience from session', () => {
      const src = fs.readFileSync(path.join(ROOT, 'jesha-pos-backend', 'src/security/seller-auth.js'), 'utf8')
      assert.ok(src.includes("pos-seller-auth"), 'Should use pos-seller-auth audience')
    })
  })
})

// ─── Pure helper: resolves seller ID following the 4-case logic ─────
// This mirrors the backend controller logic for unit testing.
function resolveSellerId(bodyUsuarioId, sessionUserId, sat, empresaId, sucursalId) {
  if (bodyUsuarioId != null && !isNaN(bodyUsuarioId) && bodyUsuarioId !== sessionUserId) {
    ...
    if (sucursalId != null && satClaims.bid != null && satClaims.bid !== sucursalId) {
