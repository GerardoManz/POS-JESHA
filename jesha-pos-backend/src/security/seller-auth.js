...
function signSellerAuthorization({ sellerId, sessionUserId, empresaId, sucursalId }) {
  ...
  if (!Number.isInteger(sucursalId) || sucursalId <= 0) {
    throw new SellerAuthError('SELLER_AUTH_BRANCH_REQUIRED', 'Seller authorization requires a valid branch (sucursalId)')
  }
...
function verifySellerAuthorization(token) {
  ...
  if (!Number.isInteger(payload.bid) || payload.bid <= 0) {
    throw new SellerAuthError('SELLER_AUTH_BRANCH_INVALID', 'Seller authorization missing or invalid branch ID')
  }
